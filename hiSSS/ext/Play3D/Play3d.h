...
#define PLAY3D_VERSION "v1.0.3-14-g9247ea8"
#define PLAY3D_BUILD_TIME "2024-03-27 09:46:12.044211"
...
namespace Play3d::Graphics
{
	...
	TextureId CreateTextureFromFile(const char* filePath, bool bGenerateMipLevels = true);
...
namespace Play3d::Graphics
{
	struct MaterialShaderKey
	{
		union {
			struct
			{
				...
				u32 m_lightCount  : 2; //! Determines the number of lights computed in the forward lighting pass.
			} m_bits;
			u32 m_value;
		};

		static constexpr u32 kUsedBits = 8;
	...
	private:
		...
		ShaderId CompileMaterialShader(const char* pNamePrefix, MaterialShaderKey& key, const char* pShaderCode);
	...
	void BeginPrimitiveBatch(bool depthEnable /*= true*/)
{
		...
		{
			s_internalState.m_pCurrentPrimitiveBatch = PrimitiveRenderSystem::Instance().AllocatePrimitiveBatch(depthEnable);
		}
	}
	...
	void DrawAxes(const Matrix4x4f& mat, float scale)
	{
		DrawLine(mat.m_column[3].xyz(), mat.m_column[3].xyz() + (mat.m_column[0].xyz() * scale), Colour::Red, Colour::Red);
		DrawLine(mat.m_column[3].xyz(), mat.m_column[3].xyz() + (mat.m_column[1].xyz() * scale), Colour::Green, Colour::Green);
		DrawLine(mat.m_column[3].xyz(), mat.m_column[3].xyz() + (mat.m_column[2].xyz() * scale), Colour::Blue, Colour::Blue);
	}
	...
	void BindGlobalTexture(u32 slot, TextureId textureId, SamplerId samplerId,
						   ShaderStageFlag::Type stageBinding, TextureBindFlags::Type bindFlags)
	{
...
extern const char* HLSL_PbrMeshShader;
extern const char* HLSL_PrimitiveBatchShader;
extern const char* HLSL_SkinnedMeshShader;
extern const char* HLSL_SkyboxShader;
extern const char* HLSL_SplitBRDFLUT;
...
namespace Play3d::Graphics
{
	...
	ShaderId Graphics_Impl::GetMaterialShader(MaterialShaderKey key)
	{
		...
		{
			m_simpleMaterialShaders[key.m_value] = CompileMaterialShader("SimpleMat", key, HLSL_MeshShader);
		}
	...
	ShaderId Graphics_Impl::GetPbrMaterialShader(MaterialShaderKey key)
	{
		...
		{
			m_pbrMaterialShaders[key.m_value] = CompileMaterialShader("PbrMat", key, HLSL_PbrMeshShader);
		}
	...
	result_t Graphics_Impl::CompileInternalShaders()
	{
...

		{
			...
			desc.m_hlslCode = HLSL_SkyboxShader;
			desc.m_flags = compilationFlags;
	...
	ShaderId Graphics_Impl::CompileMaterialShader(const char* pNamePrefix, MaterialShaderKey& key,
...

		{
			Shader* pVS = Resources::ResourceManager<Shader>::Instance().GetPtr(m_skinnedMeshVS);
				...
				D3D11_INPUT_ELEMENT_DESC vertexFormat[] = {
					...
					{"NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 2, 0, D3D11_INPUT_PER_VERTEX_DATA, 0},
					{"UV", 0, DXGI_FORMAT_R32G32_FLOAT, 3, 0, D3D11_INPUT_PER_VERTEX_DATA, 0},
					{"JOINTID", 0, DXGI_FORMAT_R8G8B8A8_UINT, 4, 0, D3D11_INPUT_PER_VERTEX_DATA, 0},
					{"JOINTWEIGHT", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 5, 0, D3D11_INPUT_PER_VERTEX_DATA, 0},
				};

				HRESULT hr = m_pDevice->CreateInputLayout(vertexFormat, 6, pVS->GetByteCode(), pVS->GetByteCodeSize(), &m_pSkinnedMeshInputLayout);
				PLAY_ASSERT_MSG(SUCCEEDED(hr), "CreateInputLayout");
		}
...
PSInput VS_Main(VSInput input)
{
	...
	output.position_w = mul(worldMtx, float4(input.position.xyz, 1.0f)).xyz;
...
#endif
	return float4(vAlbedo.rgb * saturate(vDiffuse + vAmbient), vAlbedo.w);
}

)";
//!------------------------------------- EmbeddedShaders/PbrMeshShader.hlsl -------------------------------------
const char* HLSL_PbrMeshShader = R"(
PLAY3D_PER_FRAME_CONSTANTS
...
PSInput VS_Main(VSInput input)
{
	...
	output.pos_sv = mul(viewProjectionMtx, float4(output.pos_w.xyz, 1.0f));

	output.colour = SRGBToLinear(input.colour) * vAlbedoColour;
	output.normal = mul(normalMtx, float4(input.normal.xyz, 0.0f)).xyz;
	output.uv = input.uv;
...
float4 PS_Main(PSInput input) : SV_TARGET
{
	float3 N = GetNormalDirection(input.normal, input.pos_w, input.uv);
	float3 V = normalize(viewPosition - input.pos_w);

	float3 Ca = GetAlbedoColour(input.colour, input.uv, vAlbedoColour ).rgb;

	float fOcclusion = 1.f;

	float3 vORM = GetOcclusionRoughnessMetalness(float3(fOcclusion, vPbrParams.y, vPbrParams.x), input.uv);
	...
	[unroll] for (uint i = 0; i < LIGHT_COUNT; ++i)
	{
		...
		fR += CookTorrance(Ca, Cl, vORM.y, vORM.z, N, L, V);
...
const char* HLSL_SkyboxShader = R"(
PLAY3D_PER_FRAME_CONSTANTS
...
float4 PS_Main(PSInput input) : SV_TARGET
{
	...
	float3 Csky = g_skyboxCube.Sample(g_linearSampler, N).rgb;
...
const char* HLSL_SplitBRDFLUT = R"(
...
cbuffer                                                                                   \
	FrameConstantData:                                                                                                 \
	register(PLAY3D_REG_FRAME_CONSTANTS)                                                                                                       \
	{                                                                                                                  \
		float4x4 viewMtx;                                                                                              \
...
cbuffer                                                                                    \
	DrawConstantData:                                                                                                  \
	register(PLAY3D_REG_DRAW_CONSTANTS)                                                                                                       \
	{                                                                                                                  \
		float4x4 mvpMtx;                                                                                               \
		float4x4 worldMtx;                                                                                             \
		float4x4 normalMtx;                                                                                            \
...
cbuffer                                                                                       \
	LightConstantData:                                                                                                 \
	register(PLAY3D_REG_LIGHT_CONSTANTS)                                                                                                       \
	{                                                                                                                  \
...
cbuffer                                                                                    \
	...
	register(PLAY3D_REG_MATERIAL_CONSTANTS)                                                                                                       \
	{                                                                                                                  \
...
float3 GetNormalDirection(float3 inNormal, float3 inPosW, float2 inUV)
{
	...
	normalTexture = SRGBToLinear(normalTexture);
...
{
#if USE_TEXTURE_2
	return SRGBToLinear(g_texture2.Sample(g_sampler0, inUV).rgb) * inSpecularScale;
#else
...
{
#if USE_TEXTURE_2
	return SRGBToLinear(g_texture2.Sample(g_sampler0, inUV).rgb) * inORMScale;
#else
	return inORMScale;
#endif
}
...
namespace Play3d::Graphics
{
	...
	{
		const bool bIsDepthFormat = rDesc.format == TextureFormat::DEPTH;
		...
		if (bIsRenderSurface)
		{
			if (rDesc.format != TextureFormat::DEPTH)
			{
